...
def get_all(request):
    ...
    if request.method in ['GET']:
        fields = list(map(lambda item: item.name, History._meta.fields))
        data =[]
        
        for item in History.objects.all().order_by('eventname').values(*fields):
...
def add(request):
    ...
    if request.method in ['POST']:
        try:
            ...
            vals = History.objects.filter(
                eventname=results[History._meta.pk.name]).values()

            if len(vals) > 0:
                return HttpResponse("It's exist")
            else:
                # not exist
                history_new = History(*dict(results).values())
                history_new.save(),
...
def update(request):
    ...
    if request.method in ['PUT']:
        try:
            ...
            query = History.objects.filter(eventname=body[History._meta.pk.name])
...
def delete(request, eventname):
    ...
    if request.method in ['DELETE']:
        try:
            # field validation
            query = History.objects.filter(eventname=eventname)
            if query.exists():
                query.delete()
                return HttpResponse("'{pk_id}' deleted".format(pk_id=eventname))
            else:
                return HttpResponse("")
        except Exception as error:
            print(error)
...
def find(request, pk_id):
    ...
    if request.method in ['GET']:
        fields = list(map(lambda item: item.name, History._meta.fields))
        return HttpResponse(dict(item) for item in History.objects.filter(eventname=pk_id).values(*fields))
...
def paginate(request):
    ...
    if request.method in ["GET"]:
        try:
            ...
            _obj_all = History.objects.filter(**_argument).all()
            history_list = _obj_all.order_by('eventname').values()
            paginator = Paginator(
                history_list, _limit, allow_empty_first_page=False)
